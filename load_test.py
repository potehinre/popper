...
parser.add_option("-y","--yaml",dest="yaml_file",help="yaml file conctains info for load_testing",type="str")
...
def work(channel,mps,user,statistics):
    worker=Worker(channel,mps,user,statistics)
    ...
    if options.yaml_file:
        with open(options.yaml_file) as f:
            ...
            for record in config:
                for user in generate_user(record['user_count'],record['channel_name']):
                    greenlets.append(gevent.spawn(work,record['channel_name'],record['messages_per_second'],user,statistic))
    else:
        for channel in generate_channel(options.channel_count):
            for user in generate_user(options.user_count,channel):
                greenlets.append(gevent.spawn(work,channel,options.messages_per_second,user,statistics))
